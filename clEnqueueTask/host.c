...
int main() {
	...
	queue = clCreateCommandQueue(context, devices[0], CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &ret);
	...
	// define initial particle velocities based on TGV
	...
	for (float time = 0; time < finalT; time += 2 * tstep) {
		...
		for (particle = 0; particle < NUMPART; particle++) {
			...
			ret = clSetKernelArg(kernel, 12, sizeof(int), &particle);
